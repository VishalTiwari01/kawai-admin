...
const flattenCategories = (categories: any[], level = 0) => {
  ...
  const uploadVideos = async (
    files: File[],
  ): Promise<Array<{ videoUrl: string; public_id: string }>> => {
    try {
      const formData = new FormData();
      ...
      const data = await response.json();
      return Array.isArray(data) ? data.map(v => ({ videoUrl: v.url, public_id: v.public_id })) : [{ videoUrl: data.url, public_id: data.public_id }];
    } catch (error) {
      toast({
        title: "Upload Error",
  ...
  const handleMainVideoChange = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;
    ...
    try {
      const fileArray = Array.from(files);
      const uploadedVideos = await uploadVideos(fileArray);
  ...
  const handleVariantVideoChange = async (
    variantIndex: number,
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const files = e.target.files;
    ...
    setUploadingVariantVideoIndex(variantIndex);
    try {
      const fileArray = Array.from(files);
      const uploadedVideos = await uploadVideos(fileArray);
      ...
      setFormData((prev) => {
        ...
        newVariants[variantIndex] = {
          ...newVariants[variantIndex],
          video: [
            ...(newVariants[variantIndex].video ?? []),
            ...uploadedVideos,
          ],
        };
        return { ...prev, variants: newVariants };
      ...
      toast({
        title: "Success",
        description: `${uploadedVideos.length} variant video(s) uploaded successfully.`,
      });
    } catch (error) {
      console.error("Variant video upload error:", error);
    } finally {
      setUploadingVariantVideoIndex(null);
    }
  };
  ...
  const removeVariantVideo = (variantIndex: number, videoIndex: number) => {
    setFormData((prev) => {
      ...
      newVariants[variantIndex] = {
        ...newVariants[variantIndex],
        video: (newVariants[variantIndex].video ?? []).filter(
          (_, i) => i !== videoIndex,
        ),
      };
      return { ...prev, variants: newVariants };
  ...
  return (
    <div className="fixed inset-0 bg-background/60 backdrop-blur-md z-50 flex items-start justify-center overflow-y-auto p-4 md:p-10 pt-10">
      <motion.div
      ...
      >
        <Card className="w-full shadow-2xl border-border/50 rounded-3xl overflow-hidden">
          <CardHeader className="bg-muted/30 border-b">
            <div className="flex items-center justify-between">
              <CardTitle className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/70">
                {variant?._id ? "Edit Product" : "Create New Product"}
              </CardTitle>
              <Button 
                variant="ghost" 
                size="icon" 
    ...
    {cat.name}
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g. new, featured, summer"
                />
              </div>
            </div>
            ...
            <div className="space-y-4 border-b pb-4">
              ...
              <div>
                <Label htmlFor="main-video-upload">
              ...
              <div className="space-y-4">
                {(formData.variants ?? []).map((variant, index) => (
                  <Card key={index} className="p-4 bg-muted">
                    ...
                    {/* Variant Video Section */}
                    <div className="mt-4 space-y-2">
                      <Label htmlFor={`variant-video-upload-${index}`}>
                      ...
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          accept="video/*"
                          multiple
                          onChange={(e) => handleVariantVideoChange(index, e)}
                          disabled={uploadingVariantVideoIndex === index}
                        />
                        {uploadingVariantVideoIndex === index && (
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <Loader2 className="w-4 h-4 animate-spin" />
                            Uploading...
                          </div>
                      ...
                      {(variant.video ?? []).length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {(variant.video ?? []).map((video, videoIndex) => (
                            <Badge
                              key={videoIndex}
                              variant="secondary"
                              className="flex items-center gap-1 p-1"
                            >
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  removeVariantVideo(index, videoIndex)
                                }
                                className="ml-1 hover:text-destructive"
                              >
                                <X className="w-3 h-3" />
